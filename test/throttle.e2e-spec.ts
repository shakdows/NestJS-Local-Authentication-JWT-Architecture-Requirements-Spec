...
describe('Rate limiting (e2e, SEC-RATE)', () => {
  ...
  it('SEC-RATE-03: the 6th login within 60s → 429 RATE_LIMITED with Retry-After', async () => {
    const attempt = () => http(app).post('/auth/login').send({ email: 'x@example.com', password: 'Wrong-pass1' });
    for (let i = 0; i < 5; i += 1) expect((await attempt()).status).toBe(401);
    const blocked = await attempt();
    expect(blocked.status).toBe(429);
    expect(blocked.body.error).toMatchObject({ code: 'RATE_LIMITED', message: 'Too many requests, please try again later' });
  ...
  it('SEC-RATE-02: the 6th registration within 60s → 429', async () => {
    const statuses = [];
    for (let i = 0; i < 6; i += 1) {
      statuses.push((await http(app).post('/auth/register').send({ email: `u${i}@example.com`, password: PASSWORD })).status);
    }
  ...
  it('SEC-RATE-04: refresh allows 30/min, then 429', async () => {
    const statuses = [];
    for (let i = 0; i < 31; i += 1) statuses.push((await http(app).post('/auth/refresh').send({})).status);
