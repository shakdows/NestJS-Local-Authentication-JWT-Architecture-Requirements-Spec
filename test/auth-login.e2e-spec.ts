...
import { http, login, PASSWORD, register, setStatus, withoutTimestamp } from './utils/auth-helpers.js';
...
describe('POST /auth/login (e2e)', () => {
  ...
  it('FR-LOGIN-07: valid credentials → 200 with user, tokens, tokenType and expiresIn', async () => {
    ...
    expect(Object.keys(data).sort()).toEqual(['accessToken', 'expiresIn', 'refreshToken', 'tokenType', 'user']);
    expect(data).toMatchObject({ tokenType: 'Bearer', expiresIn: 900 });
    expect(data.user).toMatchObject({ id: created.id, email: 'user@example.com', roles: ['USER'], status: 'ACTIVE' });
    expect(data.user.lastLoginAt).not.toBeNull();
    expect(JSON.stringify(res.body)).not.toMatch(/password/i);

    const access = decode<{ sub: string; sid: string; type: string; roles: string[] }>(data.accessToken);
    const refresh = decode<{ sub: string; sid: string; type: string }>(data.refreshToken);
    expect(access).toMatchObject({ sub: created.id, type: 'access', roles: ['USER'] });
    expect(refresh).toMatchObject({ sub: created.id, type: 'refresh', sid: access.sid });
  ...
  it('FR-LOGIN-06 / SEC-TOKEN-02: creates a session storing only the SHA-256 of the refresh token', async () => {
    ...
    const { refreshToken } = await login(app, 'user@example.com', PASSWORD, 'Agent/1.0');
    ...
    expect(session.refresh_token_hash).toBe(createHash('sha256').update(refreshToken).digest('hex'));
  ...
  it('two logins create two independent sessions (multi-device)', async () => {
    ...
    expect(decode<{ sid: string }>(a.accessToken).sid).not.toBe(decode<{ sid: string }>(b.accessToken).sid);
  ...
  it('FR-LOGIN-03: incorrect password and non-existent user produce identical 401 bodies', async () => {
    await register(app, 'user@example.com');
    const wrong = await http(app).post('/auth/login').send({ email: 'user@example.com', password: 'Wrong-pass1' }).expect(401);
    const unknown = await http(app).post('/auth/login').send({ email: 'nobody@example.com', password: 'Wrong-pass1' }).expect(401);
    expect(wrong.body.error).toMatchObject({ code: 'AUTH_INVALID_CREDENTIALS', message: 'Invalid email or password' });
    expect(withoutTimestamp(wrong.body)).toEqual(withoutTimestamp(unknown.body));
    const [{ count }] = await db.query('SELECT COUNT(*)::int AS count FROM auth_sessions');
    expect(count).toBe(0);
  });
  ...
  it.each(['INACTIVE', 'SUSPENDED', 'PENDING_VERIFICATION'])(
    ...
    async (status) => {
      ...
      const res = await http(app).post('/auth/login').send({ email: 'user@example.com', password: PASSWORD }).expect(403);
      expect(res.body.error).toMatchObject({ code: 'AUTH_ACCOUNT_NOT_ACTIVE', details: { status } });
    },
  );
  ...
  it('SEC-ENUM-03: inactive user with wrong password → 401 (status not revealed)', async () => {
    ...
    const res = await http(app).post('/auth/login').send({ email: 'user@example.com', password: 'Wrong-pass1' }).expect(401);
    expect(res.body.error.code).toBe('AUTH_INVALID_CREDENTIALS');
  });

  it('FR-PWD-04: login does not enforce the registration password policy', async () => {
    const res = await http(app).post('/auth/login').send({ email: 'user@example.com', password: 'short' }).expect(401);
    expect(res.body.error.code).toBe('AUTH_INVALID_CREDENTIALS');
  });
  ...
  it.each([
    ...
    ['password too long', { email: 'user@example.com', password: 'x'.repeat(129) }, 'password'],
  ...
  it('FR-LOGIN-08: updates last_login_at', async () => {
    ...
    const [{ last_login_at }] = await db.query('SELECT last_login_at FROM users WHERE id = $1', [user.id]);
