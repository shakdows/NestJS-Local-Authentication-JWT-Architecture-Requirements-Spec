...
describe('ADMIN user management (e2e)', () => {
  ...
  const as = (token: string) => ({
    get: (path: string) =>
      http(app).get(path).set('Authorization', `Bearer ${token}`),
    patch: (path: string, body: object) =>
      http(app).patch(path).set('Authorization', `Bearer ${token}`).send(body),
    delete: (path: string) =>
      http(app).delete(path).set('Authorization', `Bearer ${token}`),
  });
      ...
      const call = (token?: string) => {
        const req =
          method === 'GET'
            ? http(app).get(path)
            : method === 'PATCH'
              ? http(app).patch(path).send({})
              : http(app).delete(path);
  ...
  describe('GET /users', () => {
    ...
    it('filters by role, status and search', async () => {
      expect(
        (
          await as(adminToken).get('/users?role=ADMIN').expect(200)
        ).body.data.items.map((u: { id: string }) => u.id),
      ).toEqual([adminId]);
      expect(
        (
          await as(adminToken).get('/users?search=USER@').expect(200)
        ).body.data.items.map((u: { id: string }) => u.id),
      ).toEqual([userId]);
      expect(
        (await as(adminToken).get('/users?status=SUSPENDED').expect(200)).body
          .data.meta.total,
      ).toBe(0);
    });
  ...
  it('GET /users/:id returns one user; unknown → 404; malformed → 400', async () => {
    ...
    await as(adminToken)
      .get('/users/0b1c2d3e-4f50-4a6b-8c7d-9e0f1a2b3c4d')
      .expect(404);
  ...
  describe('PATCH /users/:id/status', () => {
    it('suspending a user blocks their existing tokens immediately; reactivating restores access', async () => {
      const res = await as(adminToken)
        .patch(`/users/${userId}/status`, { status: 'SUSPENDED' })
        .expect(200);
      expect(res.body.data.user.status).toBe('SUSPENDED');
      expect((await as(userToken).get('/auth/me')).body.error.code).toBe(
        'AUTH_ACCOUNT_NOT_ACTIVE',
      );

      await as(adminToken)
        .patch(`/users/${userId}/status`, { status: 'ACTIVE' })
        .expect(200);
    ...
    it('FR-ADMIN-06: an admin cannot change their own status', async () => {
      const res = await as(adminToken)
        .patch(`/users/${adminId}/status`, { status: 'INACTIVE' })
        .expect(403);
    ...
    it('validates the body', async () => {
      await as(adminToken)
        .patch(`/users/${userId}/status`, { status: 'BANNED' })
        .expect(400);
    ...
    it('unknown user → 404', async () => {
      await as(adminToken)
        .patch('/users/0b1c2d3e-4f50-4a6b-8c7d-9e0f1a2b3c4d/status', {
          status: 'ACTIVE',
        })
        .expect(404);
    });
  ...
  describe('PATCH /users/:id/roles', () => {
    it('promotes USER → ADMIN (effective on next request) and demotes back', async () => {
      const promoted = await as(adminToken)
        .patch(`/users/${userId}/roles`, { roles: ['ADMIN'] })
        .expect(200);
      expect(promoted.body.data.user.roles).toEqual(['USER', 'ADMIN']);
      await as(userToken).get('/users').expect(200);

      const demoted = await as(adminToken)
        .patch(`/users/${userId}/roles`, { roles: ['USER'] })
        .expect(200);
    ...
    it('FR-ADMIN-06: an admin cannot remove their own ADMIN role', async () => {
      const res = await as(adminToken)
        .patch(`/users/${adminId}/roles`, { roles: ['USER'] })
        .expect(403);
      expect(res.body.error.code).toBe('USER_SELF_MODIFICATION_FORBIDDEN');
  ...
  describe('/users/:id/sessions', () => {
    ...
    it('revokes all sessions of a user with ADMIN_REVOKED; their tokens die immediately', async () => {
      ...
      const [row] = await app
        .get(DataSource)
        .query('SELECT revoked_reason FROM auth_sessions WHERE id = $1', [sid]);
    ...
    it('unknown user → 404', async () => {
      await as(adminToken)
        .get('/users/0b1c2d3e-4f50-4a6b-8c7d-9e0f1a2b3c4d/sessions')
        .expect(404);
      await as(adminToken)
        .delete('/users/0b1c2d3e-4f50-4a6b-8c7d-9e0f1a2b3c4d/sessions')
        .expect(404);
    });
  });
});
