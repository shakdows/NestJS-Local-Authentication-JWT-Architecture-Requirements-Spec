...
export class UsersAdminService {
  ...
  async updateStatus(actorId: string, id: string, status: UserStatus): Promise<{ user: UserResponseDto }> {
    return { user: toUserResponse(await this.usersService.updateStatus(actorId, id, status)) };
  }

  async updateRoles(actorId: string, id: string, roles: Role[]): Promise<{ user: UserResponseDto }> {
    return { user: toUserResponse(await this.usersService.setRoles(actorId, id, roles)) };
  }
}
