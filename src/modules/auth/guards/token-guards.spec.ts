...
import {
  JwtRefreshGuard,
  mapRefreshTokenFailure,
} from './jwt-refresh.guard.js';
...
describe('JwtAuthGuard failure mapping (FR-GUARD-03)', () => {
  it.each([
    ...
    [
      'missing',
      undefined,
      { name: 'Error', message: 'No auth token' },
      'AUTH_TOKEN_MISSING',
    ],
    [
      'bad signature',
    ...
    [
      'alg none',
      undefined,
      { name: 'JsonWebTokenError', message: 'jwt signature is required' },
      'AUTH_TOKEN_INVALID',
    ],
  ...
  it('passes AppExceptions from the strategy through unchanged', () => {
    const notActive = new AppException(
  ...
  it('handleRequest returns the user on success and throws otherwise', () => {
    ...
    expect(guard.handleRequest(null, { id: 'u1' }, undefined)).toEqual({
      id: 'u1',
    });
    expect(() =>
...
describe('JwtRefreshGuard failure mapping (FR-REFRESH-06)', () => {
  it('maps every failure to AUTH_REFRESH_TOKEN_INVALID', () => {
    expect(mapRefreshTokenFailure(new Error('jwt expired')).code).toBe(
      'AUTH_REFRESH_TOKEN_INVALID',
    );
    expect(mapRefreshTokenFailure(undefined).code).toBe(
      'AUTH_REFRESH_TOKEN_INVALID',
    );
    const tokenInvalid = new AppException(
      HttpStatus.UNAUTHORIZED,
      ErrorCode.AUTH_TOKEN_INVALID,
    );
    expect(mapRefreshTokenFailure(tokenInvalid).code).toBe(
      'AUTH_REFRESH_TOKEN_INVALID',
    );
  });
  ...
  it('handleRequest throws the generic refresh error', () => {
    expect(() => new JwtRefreshGuard().handleRequest(null, false)).toThrow(
